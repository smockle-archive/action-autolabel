...
async function removeLabelWithVerification(issue: Issue, label: string) {
...
describe("Autolabel", () => {
  ...
  const repo: string = "action-autolabel";

  /** The number of the test issue. */
  const issueNumber: number = 1;
  ...
  beforeEach(async () => {
    if (!process.env.GH_TOKEN) {
      throw new Error(
        "Failed to retrieve a GitHub token. A GitHub token with `repo` and `org:write` scopes is required to be present in the environment as `GH_TOKEN` to run integration tests."
      );
    }
